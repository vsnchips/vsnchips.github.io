...
set winminwidth=0
set winwidth=1
exe 'vert 1resize ' . ((&columns * 54 + 42) / 85)
exe 'vert 2resize ' . ((&columns * 30 + 42) / 85)
argglobal
let s:cpo_save=&cpo
set cpo&vim
...
nnoremap <buffer> <expr> n ' j'[v:searchforward].'n'
nnoremap <buffer> <nowait> <silent> q :call startify#open_buffers(30)
...
setlocal statusline=%#SpaceVim_statusline_ia#%{SpaceVim#layers#core#statusline#mode(mode())}\ %{\ SpaceVim#layers#core#statusline#winnr(get(w:,\ \"winid\",\ winnr()))\ }\ %#SpaceVim_statusline_ia_SpaceVim_statusline_b#%#SpaceVim_statusline_b#%{\ get(w:,\ \"winwidth\",\ 150)\ <\ 26\ ?\ \"\"\ :\ (\"\ Startify\ \ \")}%{\ get(w:,\ \"winwidth\",\ 150)\ <\ 39\ ?\ \"\"\ :\ (\"\ \ startify\ \")}%{\ get(w:,\ \"winwidth\",\ 150)\ <\ 53\ ?\ \"\"\ :\ (\"\ \ ❖\ ⓢ\ \ \ \")}
